...
ИИ-ассистент для генерации ручных тест-кейсов, UI- и API-автотестов с использованием Evolution Foundation Models.
Автоматически разбирает UI-требования и OpenAPI-спецификации, генерирует ручные тесты, полноценные автотесты, выполняет ревью, автоматически исправляет найденные дефекты в тестах.
...
#### LLM-ревью и автоматическое исправление автотестов

Специально используются две модели:

- **Генератор** — модель, предобученная на кодовой базе, пишет ручные тест-кейсы и автотесты.
- **Ревизор** — модель с большим контекстным окном, получает сгенерированные тесты и требования, проверяет их и предлагает правки.

Если ревизор обнаруживает дефекты, система автоматически переписывает тест и сохраняет улучшённую версию.
...
```bash
git clone https://github.com/<your-username>/Cloud.ru-TestOps-Copilot.git
...
```
> Перед запуском установите браузеры Playwright - иначе валидатор локаторов не сможет открыть страницу.
