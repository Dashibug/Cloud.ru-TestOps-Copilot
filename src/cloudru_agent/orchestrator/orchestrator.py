...
class AgentOrchestrator:
    ...
    def __init__(self) -> None:
        ...
        self.llm = EvolutionClient()  # <--- добавили


    # --- UI ---
    def generate_ui_from_text(self, text: str, output_dir: str) -> None:
        """
        текст требований -> Evolution FM -> требования -> ручные кейсы + автотесты.
        """
        ...
        self.manual_generator.generate_ui_tests(requirements_doc, str(manual_dir), llm=self.llm)
        self.ui_auto_generator.generate(requirements_doc, str(auto_dir), llm=self.llm)
    ...
    def generate_ui_manual_tests(self, requirements_path: str, output_dir: str) -> None:
        ...
        self.manual_generator.generate_ui_tests(doc, output_dir)

    def generate_ui_automation(self, requirements_path: str, output_dir: str) -> None:
        doc = self.ui_parser.parse(Path(requirements_path))
        self.ui_auto_generator.generate(doc, output_dir)

    # --- API ---

    def generate_api_manual_tests(self, openapi_path: str, output_dir: str) -> None:
        doc = self.openapi_parser.to_requirements(Path(openapi_path))
        self.manual_generator.generate_api_tests(doc, output_dir)

    def generate_api_automation(self, openapi_path: str, output_dir: str) -> None:
        doc = self.openapi_parser.to_requirements(Path(openapi_path))
        self.api_auto_generator.generate(doc, output_dir)

    # --- Analysis ---

    def analyze_tests(self, tests_dir: str) -> None:
        tests_path = Path(tests_dir)
        coverage_report = self.coverage_analyzer.build_report(tests_path)
